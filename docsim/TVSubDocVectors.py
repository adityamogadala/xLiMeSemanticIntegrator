...
class SubtitlesVectors:
	...
	def createpklfiles(self,lang0,lang,corpus_ids,rows,word_features,X):
		...
		with open(self.path+'docsim/subtitledocvecpkl/docbivec_'+lang0+'_'+lang+'.pkl', 'wb') as outfile:
	...
	def encorpus(self,corpus_en,corpus_en_id):
		...
		with open("_sub_ids_en.txt","w") as f:
			for row in ids:
				f.write(row+"\n")
		shutil.copy2("_sub_ids_en.txt",self.path+"docsim/subtitledocvecpkl/ids_en.copy")
	def decorpus(self,corpus_de,corpus_de_id):
		...
		with open("_sub_ids_de.txt","w") as f:
			for row in ids:
				f.write(row+"\n")
		shutil.copy2("_sub_ids_de.txt",self.path+"docsim/subtitledocvecpkl/ids_de.copy")
	def itcorpus(self,corpus_it,corpus_it_id):
		...
		with open("_sub_ids_it.txt","w") as f:
			for row in ids:
				f.write(row+"\n")
		shutil.copy2("_sub_ids_it.txt",self.path+"docsim/subtitledocvecpkl/ids_it.copy")
	def escorpus(self,corpus_es,corpus_es_id):
		...
		with open("_sub_ids_es.txt","w") as f:
			for row in ids:
				f.write(row+"\n")
		shutil.copy2("_sub_ids_es.txt",self.path+"docsim/subtitledocvecpkl/ids_es.copy")
        def computevectors(self):
			...
			if self.configdict['MongoDBUserName']!="" and self.configdict['MongoDBPassword']!="":
                		...
                		collection = storedb[self.configdict['KafkaTopicSubtitles']]
